...
# all_sp <- read.csv("data/all_splist_20241218.csv")
# 
...
#                    species_folder = paste0("../data/species/key=", unique(not_available$AphiaID)[i]))
#     # mp_standardize(species = unique(not_available$AphiaID)[i],
#     #                species_list = "data/all_splist_20241218.csv",
#     #                sdm_base = rast("../data/env/current/thetao_baseline_depthsurf_mean.tif"),
#     #                species_folder = "../")
#   } else {
#     print("Not here")
...
# And then do the standardization
for (i in 1:length(unique(all_sp$AphiaID))) {
  if (!file.exists(paste0("data/species/", "key=", unique(all_sp$AphiaID)[i], ".parquet"))) {
    print("here")
    print(unique(all_sp$AphiaID)[i])
    print("okay printed")
    
    cat(unique(all_sp$AphiaID)[i], "\n")
    mp_standardize(species = unique(all_sp$AphiaID)[i],
                   species_list = "analysis/anemonefish/anemone_list.csv",
                   sdm_base = rast("../data/env/current/thetao_baseline_depthsurf_mean.tif"),
                   species_folder = "../data/raw/final_full_exports")
    # mp_standardize(species = unique(not_available$AphiaID)[i],
    #                species_list = "data/all_splist_20241218.csv",
    #                sdm_base = rast("../data/env/current/thetao_baseline_depthsurf_mean.tif"),
    #                species_folder = "../")
    print("did mp_standardize")
  } else {
    print("Not here")
    print(unique(all_sp$AphiaID)[i])
    print("okay printedddd")
    
    cat(unique(all_sp$AphiaID)[i], "done \n")
  }
}
